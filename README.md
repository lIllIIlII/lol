...
- **网页盘管理**：蓝奏云 / 奶牛快传 / 小飞机 / 城通网盘 / 文叔叔登录后，点击卡片直接进入网盘页管理文件（不再展示 Cookie）
...
1. **解析**：首页粘贴分享链接（可带提取码文案，自动识别），点击「开始解析」
   - 支持的链接形态：`pan.quark.cn/s/xx`、`drive.uc.cn/s/xx`、`pan.xunlei.com/s/xx`、`pan.baidu.com/s/xx`、`caiyun.139.com/m/i?xx`、`123pan.com/s/xx`、`*.lanzou*.com/xx`（含 lanzouw / lanzoui / lanzouc / lanpw / lanpv / lanzn 等全部家族变体）、`ilanzou.com/s/xx`（蓝奏云优享版）、`cowtransfer.com/s/xx`、`share.feijipan.com/s/xx`、`*.ctfile.com/f/xx`（城通网盘，含 545c / pipipan 镜像；文件夹 /dir/ 暂不支持）、`wenshushu.cn/f/xx`（文叔叔）
2. **浏览**：解析后进入文件列表，可进入子文件夹、多选、搜索
3. **下载**：点击文件即取直链并加入下载队列（下载页查看进度，完成后点击「打开」）
4. **网盘**：登录各平台账号（WebView 抓取 Cookie，本地 AES 加密保存），可浏览云盘、转存、创建分享；蓝奏云 / 奶牛快传 / 小飞机 / 城通网盘 / 文叔叔登录后点击卡片直接进入网页版网盘
5. **设置**：
   - `下载线程数`：按平台调整并发分片数（默认 16）
   - `下载保存目录`：默认系统下载目录，可自选 SAF 目录
   - `转存后询问下载`：解析分享转存到你的网盘后弹窗询问是否立即下载（可开关）
...
- **蓝奏云仍失败**：错误信息附逐域名诊断时，多为 VPN/代理出口被拒或运营商网络屏蔽，请关闭 VPN 或切换 Wi-Fi/流量后重试
- **转存后没有弹下载**：确认「设置 → 转存后询问下载」已开启；仅单文件转存触发（文件夹转存请到网盘页手动下载）
