...
fun DriveScreen(
    ...
    val cow = DriveAccount(
        id = "cowtransfer",
        name = "奶牛快传",
        description = cowAccount?.nickname?.takeIf { it.isNotBlank() } ?: "登录可选，支持解析下载",
        avatarText = "奶",
        isLoggedIn = cowAccount != null
    )
    val feiji = DriveAccount(
        id = "feiji",
        name = "小飞机网盘",
            ...
            onDownloadStarted = onDownloadStarted
                ...
                item(key = feiji.id) {
                    DriveAccountCard(
                        ...
                        onMoreClick = if (feiji.isLoggedIn) {
                            { showSimpleSheetFor = "feiji" }
                        } else {
                            null
                        }
