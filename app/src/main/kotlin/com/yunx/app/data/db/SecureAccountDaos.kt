...
internal object SecureAccountDaos {
    ...
    fun simple(raw: SimpleAccountDao, cipher: CredentialCipher): SimpleAccountDao = object : SimpleAccountDao {
        override fun observeAll(): Flow<List<SimpleAccountEntity>> = raw.observeAll().map { list ->
            list.mapNotNull { decryptSimple(raw, cipher, it) }
        }
        override fun observeAccount(platform: String): Flow<SimpleAccountEntity?> =
            raw.observeAccount(platform).map { value ->
                value?.let { decryptSimple(raw, cipher, it) }
            }
        override suspend fun upsert(account: SimpleAccountEntity) = withContext(Dispatchers.IO) {
            raw.upsert(encryptSimple(cipher, account))
        }
        override suspend fun getAccount(platform: String): SimpleAccountEntity? =
            raw.getAccount(platform)?.let { decryptSimple(raw, cipher, it) }
        override suspend fun clear(platform: String) = raw.clear(platform)
    }
    ...
    private suspend fun decryptSimple(raw: SimpleAccountDao, cipher: CredentialCipher, stored: SimpleAccountEntity): SimpleAccountEntity? =
        withContext(Dispatchers.IO) {
            decryptOrClear({ raw.clear(stored.platform) }) {
                val plain = stored.copy(cookie = cipher.decrypt(stored.cookie, "simple.${stored.platform}.cookie"))
                if (!cipher.isEncrypted(stored.cookie)) raw.upsert(encryptSimple(cipher, plain))
                plain
            }
        }

    private fun encryptSimple(cipher: CredentialCipher, value: SimpleAccountEntity) =
        value.copy(cookie = cipher.encrypt(value.cookie, "simple.${value.platform}.cookie"))
